...
KEY = 'AIzaSyDZuNGffQRyayTgMYoVDbzlkxHFroBOhI0'
...
def main():
    ...
    # read excel data
    data = pd.read_excel('fetchcheckpoint/data.xlsx', sheet_name='Mexico (Transfer)')
    ...
    for link in data['Link'][start:]:
        try:
            ...
            try:
                ...
                print('At index %s' % {str(data.loc[data['Link']==f'http://www.youtube.com/channel/{channel_id}'].index[0])})
                # save stopping location and aggregate data
                with open('fetchcheckpoint/index.txt', 'w') as f:
                    f.write(str(data.loc[data['Link']==f'http://www.youtube.com/channel/{channel_id}'].index[0]))
                    f.close()
